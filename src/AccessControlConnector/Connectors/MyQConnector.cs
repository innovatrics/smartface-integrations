...
namespace Innovatrics.SmartFace.Integrations.AccessControlConnector.Connectors
{
    public class MyQConnector : IAccessControlConnector
    {
        ...
        public async Task OpenAsync(StreamConfig streamConfig, string accessControlUserId = null)
        {
            ...
            this.logger.Information($"MyQ Printer: {streamConfig.TargetId} Initiating OpenAsync using email: ({accessControlUserId}), using stream mapping: {streamConfig.StreamId}");
            ...
            try
            {
                ...
                await UnlockPrinter(streamConfig.TargetId, streamConfig.StreamId, userToken);
            }
            catch (Exception ex)
            {
