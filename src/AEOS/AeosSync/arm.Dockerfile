...
ARG COMMON_MSBUILD_OPTIONS="-r linux-arm64 -p:Platform=ARM64 -p:Configuration=Release -p:SFPlatform=Docker -p:PublishReadyToRun=true"
...
ARG MSBUILD_BUILD_OPTIONS
WORKDIR /source

COPY . .
RUN dotnet restore src/AEOS/AeosSync/AeosSync.csproj
RUN dotnet publish ${MSBUILD_PUBLISH_OPTIONS} -o /build_output src/AEOS/AeosSync/AeosSync.csproj

FROM --platform=${RUNTIME_PLAFTORM} ${DOTNET_RUNTIME_IMAGE} AS tool
WORKDIR /App
COPY --from=build /build_output .
ENTRYPOINT ["dotnet", "AeosSync.dll"]