...
namespace SmartFace.GoogleCalendarsConnector.Services
{
    public class StreamGroupTracker
    {
        ...
        public StreamGroupTracker(ILogger logger, IConfiguration configuration)
        {
            ...
            _interval = TimeSpan.FromSeconds(configuration.GetValue<double>("StreamGroupTracker:IntervalSec", 15.0));
            _minPedestrians = configuration.GetValue<double>("StreamGroupTracker:MinPedestrians", 1.0);
            _minFaces = configuration.GetValue<double>("StreamGroupTracker:MinFaces", 0.0);
            _minIdentifications = configuration.GetValue<double>("StreamGroupTracker:MinIdentifications", 0.0);
